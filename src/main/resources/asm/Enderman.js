    ...
    return {
    	'Enderman': {
    		...
    		'transformer': function(classNode) {
    			...
    			if (count < 1)
    				asmapi.log("ERROR", "Failed to modify EndermanEntity: Method not found")
    			return classNode;
    		}
    	}
    }
...
function patch_func_70037_a(obj) {
	...
	if (node && node.desc == n1) {
		...
		if (node2 && node2.getOpcode() == opc.GETFIELD && node2.name == f1)
		{
			...
			asmapi.log("ERROR", "Failed to modify EndermanEntity: GETFIELD not found")
	}
	else
		asmapi.log("ERROR", "Failed to modify EndermanEntity: CHECKCAST not found")
}
